...
app.get("/scrape", function (req, res) {
	request("https://www.nytimes.com/", function (error, response, html) {
		...
		app.get("/", function (req, res) {
			Article.find({}, null, { sort: { created: -1 } }, function (err, data) {
				if (data.length === 0) {
					res.render("placeholder", { message: "There's nothing scraped yet. Please click \"Scrape For Newest Articles\" for biased news." });
				}
				else {
					res.render("index", { articles: data });
				}
		...
		app.get("/scrape", function (req, res) {
			request("https://www.nytimes.com/", function (error, response, html) {
				var $ = cheerio.load(html);
				var result = {};
				$(".meteredContent css-1i2y565").each(function (i, element) {
					var link = $(element).find("a").attr("href");
					var title = $(element).find(".h2 class css-1w0yruz esl82me0").text().trim();
					var summary = $(element).find("#div class css-53u6y8").text().trim();
					var img = $(element).parent().find("#div class css-bsn42l").find("img").attr("src");
					result.link = link;
					result.title = title;
					if (summary) {
